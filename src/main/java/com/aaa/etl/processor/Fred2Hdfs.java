...
public class Fred2Hdfs {
    ...
    // Fred Json 타입의 데이터를 EtlColumnPojo로 매핑하여 변환시키는 객체
    ...
    public void clearInputFiles(String path, String filename) throws Exception{
        ...
        if(hadoopFs.exists(hadoopPath)){
            ...
            System.out.println("Hadoop File System에서 " + hadoopPath.getName() + " 파일이 삭제되었습니다.");
        }

        File localPath = new File(path + filename);
        if (localPath.exists()){
            System.out.println("Local 환경에서 " + localPath.getName() + " 파일이 삭제되었습니다.");
        }
    }
